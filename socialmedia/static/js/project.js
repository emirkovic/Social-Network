document.addEventListener("DOMContentLoaded", function () {
        ...
        data.forEach((notification) => {
          ...
          notificationElement.innerHTML = `
                        <div class="d-flex align-items-center">
                            ...
                            }" class="rounded-circle" height="30" alt="User Avatar" />
                            <div class="ml-2">
                                <p><strong><a href="/social/profile/${
                                  notification.trigger_user
                                }/">${notification.trigger_user}</a></strong> ${
            notification.text
          }</p>
  ...
  function updateSuggestions(suggestions) {
    ...
    suggestions.forEach((suggestion) => {
      const profileImage = suggestion.profile_image
    ...
    return `
        <div class="post mt-4 p-3 bg-light rounded" id="post-${post.id}">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <strong class="ml-2">
                        <a href="/social/profile/${
                          post.username
                        }/" class="text-dark">${post.username}</a>
                    </strong>
                </div>
                ${
                  isCurrentUserPost
                    ? `
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle" type="button" id="postOptions-${
                      post.id
                    ...
                    <ul class="dropdown-menu" aria-labelledby="postOptions-${
                      post.id
                        ...
                        <li><a class="dropdown-item edit-post" href="#" data-post-id="${
                          post.id
                        }">Edit Post</a></li>
                        <li><a class="dropdown-item toggle-comments" href="#" data-post-id="${
                          post.id
                        }">${
                        ...
                        <li><a class="dropdown-item delete-post" href="#" data-post-id="${
                          post.id
                        }">Delete</a></li>
                    </ul>
                </div>`
            ...
            <div class="mt-3">
                ${
                  post.image
                    ? `<img src="${post.image}" alt="Post Image" class="img-fluid post-image" />`
                    : ""
                }
                ${
                  post.video
                    ? `<video controls class="img-fluid mt-3 post-video"><source src="${post.video}" type="video/mp4" />Your browser does not support the video tag.</video>`
                    : ""
                }
                ${
                  post.youtube_link
                    ? `<iframe width="560" height="315" src="https://www.youtube.com/embed/${post.youtube_link}" frameborder="0" allowfullscreen class="post-youtube-link"></iframe>`
                    : ""
                }
                <p class="mt-3 post-text">${post.text}</p>
                <div class="d-flex justify-content-between mt-2">
                    <div>
                        <button class="btn btn-light like-btn ${
                    ...
                    <small class="text-muted">
                        Liked by <strong id="last-liked-${
                          post.id
                        }">${post.last_liked_user}</strong> and <strong id="likes-count-${post.id}">${post.likes_count}</strong> others
                    </small>
                </div>
                <div id="comments-${
                  post.id
                }" class="comments mt-3" ${post.comments_disabled ? 'style="display: none;"' : ""}>
                    ${
                      Array.isArray(post.comments)
                        ? post.comments
                            .map(
                              (comment) => `
                        <div class="comment mb-2 d-flex" id="comment-${
                          comment.id
                        }">
                            <img src="${
                              comment.profile_image
                                ? comment.profile_image
                                : "https://via.placeholder.com/150"
                            }" alt="Profile Picture" class="rounded-circle comment-img" height="30" />
                    ...
                    <form method="post" action="/social/post/${
                      post.id
                    }/" class="comment-form mt-2" ${post.comments_disabled ? 'style="display: none;"' : ""}>
      ...
      .then((data) => {
        if (data.total_likes !== undefined) {
          ...
          document.getElementById(`likes-count-${postId}`).textContent =
            data.total_likes;
          document.getElementById(`last-liked-${postId}`).textContent =
            data.last_liked_user || "";
        }
      })
  ...
  function openEditPostModal(postId) {
    ...
    const image = post.querySelector(".post-image")
      ? post.querySelector(".post-image").src
      : "";
    const video = post.querySelector(".post-video")
      ? post.querySelector(".post-video").src
      : "";
    const youtubeLink = post.querySelector(".post-youtube-link")
      ? post.querySelector(".post-youtube-link").src
      : "";
  ...
  function handleToggleComments(button) {
    ...
    fetch(`/social/disable_comments/${postId}/`, {
      ...
      .then((data) => {
        if (data.success) {
          const newState = data.comments_disabled
            ? "Enable Comments"
            : "Disable Comments";
    ...
    document.querySelectorAll(".comment-form").forEach((form) => {
      form.addEventListener("submit", function (event) {
        ...
        if (words.length > maxWords) {
          ...
          event.preventDefault();
          showAlert(
            `This field cannot contain more than ${maxChars} characters.`
          );
        ...
        if (
          form
            ...
            .textContent.includes("Enable Comments")
        ) {
          event.preventDefault();
          showAlert("Comments have been disabled for this post.");
        }
  ...
  if (postForm) {
    postForm.addEventListener("submit", function (event) {
      ...
      const youtubeLinkField = document.querySelector(
      ...
      if (words.length > maxWords) {
        ...
        (!imageField.files.length &&
          !videoField.files.length &&
          !youtubeLinkField.value.trim())
      ) {
        event.preventDefault();
        showAlert(
          "You must provide text, an image, a video, or a YouTube link."
        );
      }
    });
  }
          ...
          if (data.success) {
            ...
            if (data.post.image) {
              ...
              if (!img) {
                ...
                post.insertBefore(
                  img,
                  post.querySelector(
                    ".post-video, .post-youtube-link, .post-text"
                  )
                );
              }
            ...
            if (data.post.video) {
              ...
              if (!video) {
                ...
                post.insertBefore(
                  video,
                  post.querySelector(".post-youtube-link, .post-text")
                );
              }
            ...
            editPostForm.reset();
            const editPostModal = bootstrap.Modal.getInstance(
              document.getElementById("editPostModal")
            );
            editPostModal.hide();
